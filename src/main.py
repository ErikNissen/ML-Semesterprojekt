...
class Table:
    ...
    def draw_visited(self):
        for pos in self.visited:
            self.draw_text_in_cell(math.pow(0.91, self.steps), pos)
            x, y = pos
    ...
    def draw_text_in_cell(self, text, pos):
        ...
        text = font.render(f"{text}.4f", True, pygame.Color('black'))
        text_rect = text.get_rect(center=(pos[0] * self.cell_size + self.cell_size // 2, pos[1] * self.cell_size + self.cell_size // 2))
    ...
    def draw_steps(self):
        ...
        text_rect = text.get_rect(center=(self.width * self.cell_size // 2, self.height * self.cell_size + self.cell_size // 2))
        self.screen.blit(text, text_rect)
